    ...
    return (
        <div id="register-form-container" className="form-container flex items-center justify-center bg-white relative font-bree p-8">
            ...
            <form id="register-form" className="space-y-6 w-[90%] mx-auto" onSubmit={handleSubmit(onSubmit)}>
                ...
                {...register('nombre', { required: 'Required field' })} className="w-full px-4 py-1 rounded-md border border-gray-300 bg-[#EEF0FF] text-[#00174B] focus:outline-none focus:ring-1"/>
                {errors.nombre && <p className="text-red-600 "> {errors.nombre.message}</p>}
                <input id="input-apellidos" type="text" placeholder="Apellidos:" 
                {...register('apellidos', { required: 'Required field' })} className="w-full px-4 py-1 rounded-md border border-gray-300 bg-[#EEF0FF] text-[#00174B] focus:outline-none focus:ring-1"/>
                {errors.apellidos && <p className="text-red-600 text-sm"> {errors.apellidos.message}</p>}
                <input id="input-dni" type="text" placeholder="DNI:" 
                {...register('dni', { required: 'Required field', pattern: { value: /^\d{8}[A-Za-z]$/, message: 'Formato de DNI no válido (8 cifras y una letra)' }})} className="w-full px-4 py-1 rounded-md border border-gray-300 bg-[#EEF0FF] text-[#00174B] focus:outline-none focus:ring-1"/> 
                ...
                {...register('email', { required: 'Required field', pattern: { value: /^[^@]+@[^@]+\.[a-zA-Z]{2,}$/, message: 'Correo no válido' }})} className="w-full px-4 py-1 rounded-md border border-gray-300 bg-[#EEF0FF] text-[#00174B] focus:outline-none focus:ring-1"/> 
                {errors.email && <p className="text-red-600 text-sm"> {errors.email.message}</p>}
                <input id="input-password" type="password" placeholder="Contraseña:" 
                {...register('password', { required: 'Required field', minLength: { value: 8, message: 'La contraseña debe tener al menos 8 caracteres' }})}  className="w-full px-4 py-1 rounded-md border border-gray-300 bg-[#EEF0FF] text-[#00174B] focus:outline-none focus:ring-1"/> 
